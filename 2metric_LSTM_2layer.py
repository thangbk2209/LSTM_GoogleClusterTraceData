...
for batch_size in batch_size_array: 
	...
	testNotInverseDf.to_csv('results/2layer_4-2neu/testPredict_batchsize=%s.csv'%(batch_size), index=False, header=None)
	testDf = pd.DataFrame(np.array(testPredictInverse))
	testDf.to_csv('results/2layer_4-2neu/testPredictInverse_batchsize=%s.csv'%(batch_size), index=False, header=None)
	errorScore=[]
