...
# df = read_csv('data/Fuzzy_data_resource_JobId_6336594489_5minutes.csv', header=None, index_col=False, names=colnames, usecols=[1], engine='python')
...
model = Sequential()
model.add(LSTM(4, return_sequences=True,input_shape=(look_back, 1)))
...
trainDf.to_csv('results/NN/many2onetrainPredict.csv', index=False, header=None)

testDf = pd.DataFrame(np.array(testPredict))
testDf.to_csv('results/NN/many2onetestPredict.csv', index=False, header=None)
...
plt.savefig('results/NN/CPUpredict_adam_many2one.png')
pp = PdfPages('results/NN/predictCPU_adam_many2one.pdf')
pp.savefig()
pp.close()
# plt.show()