...
for batch_size in batch_size_array: 
	...
	model = Sequential()
	model.add(LSTM(512,return_sequences=True, activation = 'relu',input_shape=(2, 1)))
	model.add(LSTM(4))
	model.add(Dense(1))
	...
	# plt.show()
	plt.savefig('results/1layer_512neu/history_batchsize=%s.png'%(batch_size))
	...
	testDf.to_csv('results/1layer_512neu/testPredictInverse_batchsize=%s.csv'%(batch_size), index=False, header=None)
	errorScore=[]
	errorScore.append(testScoreRMSE)
	errorScore.append(testScoreMAE)
	errorDf = pd.DataFrame(np.array(errorScore))
	errorDf.to_csv('results/1layer_512neu/error_batchsize=%s.csv'%(batch_size), index=False, header=None)